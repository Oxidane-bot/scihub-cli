...
Uses the Europe PMC REST search API with an Open Access filter to resolve
DOIs to full text PDF links.
"""
...
class EuropePMCOASource(PaperSource):
    """Europe PMC open-access source (OA-filtered query)."""
    ...
    def get_pdf_url(self, doi: str) -> str | None:
        ...
        if pdf_url:
            if self._should_skip_pdf_url(pdf_url):
                ...
                return None
            logger.info(f"[Europe PMC OA] Found OA paper: {doi}")
            logger.debug(f"[Europe PMC OA] PDF URL: {pdf_url}")
            return pdf_url
        return None
    ...
    def _fetch_metadata(self, doi: str) -> dict[str, Any] | None:
        ...
        try:
            metadata = retry_with_classification(
                _attempt_fetch, self.retry_config, f"Europe PMC OA API for {doi}"
            )
